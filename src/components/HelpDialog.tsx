      ...
      >
        ...
        {section === "manual" ? (
          <div className="help-content help-manual">
            ...
            <div><h3>2. 書き込む・押印する</h3><p>上の道具から文字、チェック、画像、図形、ペンなどを選びます。図形は用紙をドラッグした大きさで配置でき、クリックなら従来サイズで置けます。蛍光ペンの端は初期値が角で、丸にも変更できます。印鑑は名前で作るか画像を登録でき、クリックした位置が印鑑の中心になります。配置後は選択して位置や大きさを変更できます。</p></div>
            <div><h3>3. ページを整える</h3><p>PDFを開いたまま別のPDFを左のページ一覧へドロップすると、緑の線の位置にその全ページを結合できます。複数ファイルもまとめてドロップでき、ファイル順に入ります。既存ページはドラッグで並べ替え、右クリックで操作できます。表示中のページのすぐ上にも、前後への移動・右回転・削除があります。</p></div>
