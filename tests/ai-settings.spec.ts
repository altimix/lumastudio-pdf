...
async function desktopFixture(page: Page, options: { canStore?: boolean; saved?: boolean; failSave?: boolean; corrupt?: boolean } = {}) {
  await page.addInitScript(({ options, fakeKey }) => {
    const fallback = { available: true, model: 'gpt-environment', source: 'environment' as const, saved: false, warning: '' };
    let current = options.saved ? { ...fallback, model: 'gpt-saved', source: 'saved' as 'saved' | 'environment', saved: true } : fallback;
    ...
    const state = { saveCalls: 0, removeCalls: 0, receivedExpectedKey: false, savedModel: '' };
    ...
    Object.defineProperty(window, 'lumaDesktop', { value: {
      ...
      saveAiSettings: async ({ key, model }: { key: string; model: string }) => {
        ...
        current = { ...current, source: 'saved', model, saved: true };
...
test('デスクトップのAI設定はキーを伏せて受け渡し、保存後と再表示時に入力を残さない', async ({ page }) => {
  ...
  await dialog.getByLabel('利用モデル', { exact: true }).fill('gpt-5.4-mini');
  ...
  expect(await page.evaluate(() => (window as unknown as { aiSettingsTest: unknown }).aiSettingsTest)).toEqual({
    saveCalls: 1, removeCalls: 0, receivedExpectedKey: true, savedModel: 'gpt-5.4-mini',
  });
  await dialog.getByRole('button', { name: 'AI設定を閉じる', exact: true }).click();
  await openSettings(page);
  await expect(key).toHaveValue('');
  await expect(dialog.getByLabel('利用モデル', { exact: true })).toHaveValue('gpt-5.4-mini');
...
test('保存したキーを削除すると環境設定とモデル表示へ戻る', async ({ page }) => {
  await desktopFixture(page, { saved: true });
  await page.goto('/');
  const dialog = await openSettings(page);
  await expect(dialog.getByLabel('利用モデル', { exact: true })).toHaveValue('gpt-saved');
  ...
  await expect(dialog.getByLabel('利用モデル', { exact: true })).toHaveValue('gpt-environment');
  await expect(dialog.getByLabel('OpenAI APIキー', { exact: true })).toHaveValue('');
  await expect(dialog.getByRole('button', { name: '保存したキーを削除', exact: true })).toBeDisabled();
...
test('OSの暗号化を利用できない環境ではキーの登録を無効にする', async ({ page }) => {
  ...
  await page.goto('/');
  const dialog = await openSettings(page);
  await expect(dialog).toContainText('この環境ではキーを安全に保存できません');
...
test('暗号化された保存設定を読み込めない場合でも端末から削除できる', async ({ page }) => {
  ...
  await expect(dialog.getByLabel('OpenAI APIキー', { exact: true })).toBeDisabled();
  const remove = dialog.getByRole('button', { name: '保存したキーを削除', exact: true });
