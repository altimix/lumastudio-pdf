...
try {
  ...
  assert.equal(structured.softwareVersion, '0.3.0');
  ...
  const release = 'https://github.com/altimix/lumastudio-pdf/releases/download/v0.3.0/';
  const downloads = {
    'windows-setup': 'LumaStudio-PDF-0.3.0-windows-x64-setup.exe',
    'windows-portable': 'LumaStudio-PDF-0.3.0-windows-x64-portable.exe',
    'mac-arm64': 'LumaStudio-PDF-0.3.0-macos-arm64.dmg',
    'mac-x64': 'LumaStudio-PDF-0.3.0-macos-x64.dmg',
  ...
  assert.equal(await mobile.locator('.mobile-nav nav a').count(), 5);
  ...
  for (const [path, expected] of [
    ['/robots.txt', 200], ['/sitemap.xml', 200], ['/assets/favicon.svg', 200],
    ['/assets/editor-v0.3.0.png', 200], ['/assets/ando2026.png', 200], ['/missing-page', 404],
  ]) {
