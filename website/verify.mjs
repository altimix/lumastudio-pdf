...
try {
  ...
  assert.match(await desktop.locator('.window-topline').innerText(), /v1\.0\.4/);
  assert.match(await desktop.locator('#workspace-caption').innerText(), /v1\.0\.4/);
  ...
  assert.equal(structured.softwareVersion, '1.0.4');
  ...
  const release = 'https://github.com/altimix/lumastudio-pdf/releases/download/v1.0.4/';
  const downloads = {
    'windows-portable': 'LumaStudio-PDF-1.0.4-windows-x64-portable.exe',
    'mac-arm64': 'LumaStudio-PDF-1.0.4-macos-arm64.zip',
    'mac-x64': 'LumaStudio-PDF-1.0.4-macos-x64.zip',
  ...
  for (const [path, expected] of [
    ...
    ['/assets/editor-v1.0.4.png', 200], ['/assets/ando2026.png', 200], ['/missing-page', 404],
  ]) {
